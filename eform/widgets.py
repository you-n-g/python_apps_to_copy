...
'''
    ...
    class Media:
        js = ('%scommon/js/highslide-with-gallery.min.js' % settings.STATIC_URL, '%scommon/js/image_display_header.js' % settings.STATIC_URL)
        css = {
    ...
    def render(self, name, value, attrs={}):
        ...
        return mark_safe(u"<input type='text' %s ></input> \
            <script type='text/javascript'>$(function(){\
                $.tools.dateinput.localize('zh', {months: '一月,二月,三月,四月,五月,六月,七月,八月,九月,十月,十一月,十二月',shortMonths:'一月,二月,三月,四月,五月,六月,七月,八月,九月,十月,十一月,十二月',days:'星期日,星期一,星期二,星期三,星期四,星期五,星期六',shortDays:'周日,周一,周二,周三,周四,周五,周六'});\
                $(':input[name=%s]').dateinput({format:'yyyy-mm-dd',selectors: true, speed: 'fast', value:'%s' ,yearRange:%s,lang:'zh',firstDay: 1 });})\
...
class TimeWidget(forms.TimeInput):
    ...
    def render(self, name, value, attrs={}):
        ...
        return mark_safe(u"<input type='text' %s ></input>\
            ...
            })</script>"%(flatatt(final_attrs), name, step, start_time, end_time))
